.PHONY: help up up-d down down-v build rebuild restart logs logs-web logs-db console bash db-create db-migrate db-rollback db-seed db-reset db-setup db-console test-setup test coverage routes clean ps exec
...
test-setup: ## Create and migrate test database
	docker-compose exec -T web bin/rails db:create db:migrate RAILS_ENV=test

test: ## Run tests
	docker-compose exec -T web bin/rails test

coverage: ## Run tests with coverage report (output in coverage/index.html)
	docker-compose exec -T -e COVERAGE=1 web bin/rails test
